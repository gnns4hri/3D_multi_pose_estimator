...
neck_idx = joint_list.index(parameters.neck_id)
left_idx = joint_list.index(parameters.leftshoulder_id)
right_idx = joint_list.index(parameters.rightshoulder_id)
number_of_cameras = len(parameters.cameras)
...
def compute_error(parameters, joints, raw_inputs, orig_inputs, outputs, batch_size, camera_d_transforms, camera_matrices, distortion_coefficients):
    ...
    for joint_idx in range(len(joints)):
        ...
        for cam_idx, camera in enumerate(parameters.cameras):
            ...
            begin = len(joints) * parameters.numbers_per_joint * cam_idx + joint_idx * parameters.numbers_per_joint + 1
            end = begin + 2
            ...
            begin = len(joints) * parameters.numbers_per_joint * cam_idx + joint_idx * parameters.numbers_per_joint
            end = begin + 1
...
if __name__ == '__main__':
    ...
    for cam_idx, cam in enumerate(parameters.cameras):
        ...
        camera_matrices.append(Variable(camera_matrix(cam_idx), requires_grad=optimise_matrices))
        distortion_coefficients.append(Variable(get_distortion_coefficients(cam_idx), requires_grad=optimise_matrices))
    ...
    for epoch in range(0, epochs):
        ...
        for mini_batch, data_inputs in enumerate(train_dataloader, 0):
            ...
            columns = []
            for cam_idx, camera in enumerate(parameters.cameras):
                for item in range(len(joint_list)):
                    columns += [cam_idx*len(joint_list)*parameters.numbers_per_joint + item*parameters.numbers_per_joint + x for x in range(parameters.numbers_per_joint)]
            inputs = raw_inputs[:, columns].to(device)
            ...
            for batch, valid_data in enumerate(valid_dataloader):
                with torch.no_grad():
                    ...
                    for cam_idx, camera in enumerate(parameters.cameras):
                        for item in range(len(joint_list)):
                            columns += [cam_idx * len(
                                joint_list) * parameters.numbers_per_joint + item * parameters.numbers_per_joint + x
                                        for x in range(parameters.numbers_per_joint)]
                    inputs = raw_inputs[:, columns].to(device)
                    this_batch_size = raw_inputs.size()[0]
